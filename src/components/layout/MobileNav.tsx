...
export function MobileNav({ currentPath = "/" }: MobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-6 left-4 right-4 md:hidden z-50">
      <div className="bg-background/20 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl shadow-primary/20 hover:shadow-primary/30 transition-all duration-300">
        <div className="flex items-center justify-around px-1.5 py-1.5 gap-1">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                to={item.href}
                className="flex flex-col items-center justify-center space-y-0 px-1 py-1 transition-all duration-300 text-foreground/80 focus:outline-none"
              >
                <div className={cn(
